...
public class Helper {
        ...
        throws AcsJException 
    {
        ...
        do {
            ...
            if (retValue == null) {
                ...
                // but only because we use the TAO extensions that supports named NCs.
                try {
                    retValue = createNotificationChannel(channelName, channelKind, notifyFactoryName);
                } catch (NameAlreadyUsed ex) {
                    m_logger.log(Level.INFO, "NC '" + channelName + "' seem to be getting created.. will try to get it in " + retrySleepSec + " sec.", ex);
                    try {
     ...
     * <p>
     * Should only be invoked when the channel this supplier is attempting to connect to
     * does not exist.
...
//      final String standardEventFactoryId = org.omg.CosNotifyChannelAdmin.EventChannelFactoryHelper.id();
        ...
        try {
            ...
            org.omg.CORBA.Object notifyFactoryObj = getNamingService().resolve(t_NameFactory);
            m_logger.info("Resolved " + notifyFactoryName + " from NS");
            try {
                // check interfaces
//              try {
//                  org.omg.CORBA.Object obj = notifyFactoryObj._get_interface_def();
//                  if (obj == null) {
//                      m_logger.info("got a null for notifyFactoryObj._get_interface_def()");
//                  }
//                  else {
//                      InterfaceDef idef = InterfaceDefHelper.narrow(obj);
//                      if (idef == null) {
//                          m_logger.info("got a null from InterfaceDefHelper.narrow");
//                      }
//                      else {
//                          m_logger.info("idef.absolute_name(): " + idef.absolute_name());
//                      }
//                  }
//              } catch (Exception ex) {
//                  ex.printStackTrace();
//              }
                ...
                if (notifyFactoryObj._is_a("IDL:omg.org/CosNotifyChannelAdmin/EventChannelFactory:1.0")) {
                    m_logger.info("notifyFactory is a IDL:omg.org/CosNotifyChannelAdmin/EventChannelFactory");
                }
                else {
                    m_logger.info("notifyFactory is NOT a IDL:omg.org/CosNotifyChannelAdmin/EventChannelFactory");
                }
                if (notifyFactoryObj._is_a("IDL:sandia.gov/NotifyMonitoringExt/EventChannelFactory:1.0")) {
                    m_logger.info("notifyFactory is a IDL:sandia.gov/NotifyMonitoringExt/EventChannelFactory");
                }
                else {
                    m_logger.info("notifyFactory is NOT a IDL:sandia.gov/NotifyMonitoringExt/EventChannelFactory");
                }
                
                notifyFactory = EventChannelFactoryHelper.narrow(notifyFactoryObj);
            } catch (BAD_PARAM ex) {
                // let's try if at least the narrow to the regular corba event factory works
                try {
                    org.omg.CosNotifyChannelAdmin.EventChannelFactory notifyFactoryOld;
                    notifyFactoryOld = org.omg.CosNotifyChannelAdmin.EventChannelFactoryHelper.narrow(notifyFactoryObj);
                    // regular factory was OK, thus we complain about missing NotifyMonitoringExt subtype
                    AcsJNarrowFailedEx ex2 = new AcsJNarrowFailedEx();
                    ex2.setNarrowType("IDL:sandia.gov/NotifyMonitoringExt/EventChannelFactory:1.0");
                    m_logger.info("Narrow of extended NotifyMonitoringExt EventChannelFactory failed, a try with CosNotifyChannelAdmin will made" + ex2);
                    //throw ex2;
                } 
                catch (BAD_PARAM ex3) {
                    // not even the narrow to the regular type worked, which is even worse. Thus we put that in our exception.
                    AcsJNarrowFailedEx ex4 = new AcsJNarrowFailedEx();
                    ex4.setNarrowType("IDL:omg.org/CosEventChannelAdmin/EventChannelFactory:1.0");
                    throw ex4;
                }
            }
            ...
            try {
                ...
                if (timeLastError == null) {
                    m_logger.log(AcsLogLevel.CONFIG, "Config issue for channel '" + channelName + "'. " + 
                            "Failed to get MACI/Channels DAO from CDB, using default notification service.");
                }
            }
        }
