...
public class BACIRemoteAccess implements Runnable, RemoteAccess {
	...
	private class ServerMessage extends Thread {
		...
		public void run() {
			if (hldr == null)
				return;
			else if (hldr.code == hldr.MSG) {
				notifier.reportMessage(hldr.message);
			} else if (hldr.code == hldr.ERR) {
	...
	private class ClientImpl extends ClientPOA {
		...
		public void COBs_unavailable(String[] cob_names) {
		}
		public void COBs_available(COBInfo[] cobs) {
		}
		public boolean ping() {
			return true;
		}
	...
	private class CallbackImpl
		...
		public void invoke(org.omg.CORBA.ServerRequest request) {
			...
			OperationDef odef =
				(OperationDef) descriptions.get(createHashKey(callbackID, op));
				...
				synchronized (descriptions) {
					notifier.reportDebug(
						...
						"Added 'OperationDef' for '"
							...
							+ op
							+ "()' to cache.");
					descriptions.put(createHashKey(callbackID, op), odef);
				}
			}
			notifier.reportDebug(
				"BACIRemoteAccess$CallbackImpl::invoke",
			...
			try {
				NVList list =
					orb.create_operation_list((org.omg.CORBA.Object) odef);
	...
	public void destroy() {
		...
		synchronized (connected) {
			...
			notifier.reportDebug(
				...
				"Releasing COBs on Manager, " + array.length + " COBs total.");
			try {
				try {
					manager.release_COBs(handle, array);
				} catch (Exception e) {
					// try others
				}
			} catch (Exception e) {
				notifier.reportError("Cannot release COBs.", e);
			}
			connected.clear();
		}
		synchronized (descriptions) {
	...
	synchronized void disconnect(Introspectable target) {
		...
		if (baciIntrospector.isDevice(baciNode.getCORBARef())) {
			...
			notifier.reportDebug(
				...
				"Disconnecting from '" + target.getName() + "'.");
			manager.release_COB(handle, (String) baciNode.getUserObject());
		}
	}
	/**
	 * Insert the method's description here.
	...
	private synchronized BACITreeDataNode[] explodeDomainNode(BACITreeDataNode n) {
		...
		COBInfo[] infos = new COBInfo[n.childrenHolder.size()];
			...
			*/
			...
			if (n.domainRemainder.equals(cmp)) {
				...
				if (!domain.endsWith("/"))
					domain = domain + "/";
				int index =
					domain.substring(n.domainRemainder.length()).indexOf('/');
				String dpart =
					domain.substring(
	...
	private synchronized OETreeNode[] explodeRootNode() {
		...
		infos = manager.get_COB_info(handle, handles, "*", "*", false);
	...
	private synchronized OETreeNode[] explodeTypeNode(BACITreeDataNode node) {
		...
		for (int i = 0; i < retVal.length; i++) {
			...
			if (cob == null)
				throw new IllegalArgumentException("COB name is null " + curl);
			retVal[i] =
				new BACIRemoteNode(
					DEVICE,
					cob,
	...
	Attribute[] getAttributes(BACIRemote target) {
		...
		Attribute[] retVal = (Attribute[]) attributeIntrospected.get(id);
		//	System.out.println("Step 3");
		if (retVal == null || !bufferDescs) {
			notifier.reportMessage(
	...
	private FullInterfaceDescription getIFDesc(String id) {
		FullInterfaceDescription fid =
			(FullInterfaceDescription) interfaceDescriptions.get(id);
		if (fid == null || !bufferDescs) {
			notifier.reportDebug(
	...
	Operation[] getOperations(BACIRemote target) {
		...
		String id = ((BACIRemote) target).getIFDesc().id;
		Operation[] retVal = (Operation[]) operationsIntrospected.get(id);
		if (retVal == null || !bufferDescs) {
			notifier.reportMessage(
	...
	public void initialize() {
		...
		notifier.reportDebug(
			"BACIRemoteAccess::initialize",
			"Startup using STRICT flag: " + strict);
		// ORB stanza
		...
		try {
			...
			((com.ooc.CORBA.ORB) orb).register_initial_reference(
				"InterfaceRepository",
				repRef);
			((com.ooc.CORBA.ORB) orb).register_initial_reference(
				"DefaultRepository",
	...
	private void internalManagerConnect(BACIRemoteNode baciNode) {
		...
		notifier.reportDebug(
			...
			"Requesting COB: " + curl + "', activate = true");
		notifier.reportMessage("Connecting to '" + curl + "'.");
		IntHolder status = new IntHolder();
		org.omg.CORBA.Object obj = manager.get_COB(handle, curl, true, status);
		notifier.reportDebug(
			"BACIRemoteAccess::internalManagerConnect",
			"Manager returns ObjectInfo code = " + status.value + ".");
		if (obj == null || status.value != Manager.COB_ACTIVATED) {
			notifier.reportError("Connection to COB '" + curl + "' failed.");
			return;
		}
		...
		*/
		try {
			notifier.reportDebug(
				"BACIRemoteAccess::internalManagerConnect",
				"Querying COB '" + curl + "' for CORBA type id.");

			//*** todo: use COBInfo from previous call..
			//*** GCH TEST CODE
			COBInfo[] cobInfos =
				manager.get_COB_info(handle, new int[0], curl, "*", false);
			if (cobInfos.length != 1) {
				throw new IllegalStateException(
					"Manager did not return valid COB_info for '"
						+ curl
						+ "'.");
			}
			...
			*/
			...
			notifier.reportError(
				"Failed to connect to IR, releasing COB on Manager, if needed.",
				e);
			if (manager != null && obj != null)
				manager.release_COB(handle, curl);
