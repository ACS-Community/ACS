...
public class LogEntryTable extends javax.swing.JTable
{
	...
	private class ColumnPopupMenu extends javax.swing.JPopupMenu implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource()==copyClipboard) {
				...
				if (!selectionModel.isSelectionEmpty()) {
					...
					for (int i=selectionModel.getMinSelectionIndex(); 
						...
						if (!selectionModel.isSelectedIndex(i)) {
							...
							LogEntryXML log = getLCModel().getVisibleLogEntry(row);
				...
				LogEntryXML logEntry = getLCModel().getVisibleLogEntry(row);
	...
	private class GroupMenu extends javax.swing.JMenu implements ActionListener
	{
		...
		public void rebuild()
		{
			...
			for (int i = 1; i < n; i++)
			{
				...
				{
					rb = newItem(LogEntryXML.getFieldDescription(i - 1), groupGroup, i - 1);
					columnCount++;
					if (i == s)
						rb.setSelected(true);
				}
	...
	private class SortMenu extends javax.swing.JMenu implements ActionListener
	{
		...
		public void rebuild()
		{
			...
			for (int i = 1; i < n; i++)
			{
				if (let.visibleColumns[i])
				{
					rb = newItem(LogEntryXML.getFieldDescription(i - 1), sortGroup, i - 1);
					columnCount++;
					if (i == s)
						rb.setSelected(true);
				}
	...
	public org.w3c.dom.Node getExtraInfo()
	{
		...
		LogEntryXML additionalInfo = getLCModel().getVisibleLogEntry(index);
	...
	private String getCellStringContent(int row, int col) {
		...
		if (value instanceof Date) {
			...
			if (getColumnName(col).compareTo(LogEntryXML.getFieldDescription(LogEntryXML.FIELD_ENTRYTYPE))==0) {
	...
	private void initialize()
	{
		...
		tc = tcm.getColumn(LogEntryXML.FIELD_ENTRYTYPE + 2);
		tc.setCellRenderer(new EntryTypeRenderer(LogTypeHelper.getAllIcons()));

		tc = tcm.getColumn(LogEntryXML.FIELD_TIMESTAMP + 2);
		...
		for (int i = 0; i < n; i++)
		{
			...
			if (i == LogEntryXML.FIELD_LOGMESSAGE+2)
			{
		...
		hideColumn(LogEntryXML.FIELD_LINE+2);
		hideColumn(LogEntryXML.FIELD_ROUTINE+2);
		hideColumn(LogEntryXML.FIELD_HOST+2);
		hideColumn(LogEntryXML.FIELD_PROCESS+2);
		hideColumn(LogEntryXML.FIELD_CONTEXT+2);
		hideColumn(LogEntryXML.FIELD_THREAD+2);
		hideColumn(LogEntryXML.FIELD_LOGID+2);
		hideColumn(LogEntryXML.FIELD_PRIORITY+2);
		hideColumn(LogEntryXML.FIELD_URI+2);
        hideColumn(LogEntryXML.FIELD_STACKID+2);
        hideColumn(LogEntryXML.FIELD_FILE+2);
        hideColumn(LogEntryXML.FIELD_STACKLEVEL+2);
	...
	{
		int nFields = LogEntryXML.NUMBER_OF_FIELDS;
		...
		{
			fieldNames[i] = LogEntryXML.getFieldDescription(i);
		}
