...
*
* "@(#) $Id: AES2H.xslt,v 1.17 2006/04/13 18:13:12 bjeram Exp $"
...
#include "acserrExceptionManager.h"
		...
		<xsl:if test="count(//Code)+count(//ErrorCode) > 0">
...
-->
			...
			<xsl:for-each select="Code">
				...
				<xsl:text>: public CompletionImpl
{
	...
	public:
				...
				<xsl:value-of select="$CompName"/>
				<xsl:text> () : CompletionImpl(m_etype, m_code) {}				
 };
				...
				<xsl:text>: public CompletionImpl
{
	...
	public:
		...
		CompletionImpl(m_etype, m_code, file, line, routine, m_shortDescription, severity) {}
					
	</xsl:text>
				<xsl:value-of select="$CompName"/>
				<xsl:text> (ACSErr::Completion *pc, const char* file, int line, const char* routine, ACSErr::Severity severity=DEFAULT_SEVERITY) : 
		CompletionImpl(*pc, m_etype, m_code, file, line, routine, m_shortDescription, severity) {}
		...
		CompletionImpl(pc, m_etype, m_code, file, line, routine, m_shortDescription, severity) {}
				...
				<xsl:value-of select="$CompName"/>
				<xsl:text> (CompletionImpl *pc, const char* file, int line, const char* routine, ACSErr::Severity severity=DEFAULT_SEVERITY) : 
		CompletionImpl(pc, m_etype, m_code, file, line, routine, m_shortDescription, severity) {}
		...
		CompletionImpl(et, m_etype, m_code, file, line, routine, m_shortDescription, severity) {}	
						
	template &lt;class T &gt;
	</xsl:text>
				<xsl:value-of select="$CompName"/>
				<xsl:text> (const T &amp;pe, const char* file, int line, const char* routine, ACSErr::Severity severity=DEFAULT_SEVERITY) : 
		CompletionImpl(ETHolder&lt;T&gt;(pe).getErrorTrace(), m_etype, m_code, file, line, routine, m_shortDescription, severity) {}
</xsl:text>
<!--             *******************************************************                                    members -->
<xsl:apply-templates/>
<xsl:text>
