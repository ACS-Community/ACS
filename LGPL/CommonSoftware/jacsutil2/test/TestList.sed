...
s/Mininum = .* Maximum = .*/Mininum = Date1, Maximum = Date2/g
s/New queue file created: StringsQueue.*.tmp/New queue file created: StringsQueuexxxxxxx.tmp/g
s/New queue file created with name: .*.tmp/New queue file created with name: NAME/g
s/Notification [0-9]* received for file .*/Notification N received for file NAME/g
s/Stopped pushing; [0-9]* files used by the cache to store [0-9]* entries/Stopped pushing; N files used by the cache to store X entries/g
s/StringsQueue.*/StringsQueueXXXX.tmp/g
