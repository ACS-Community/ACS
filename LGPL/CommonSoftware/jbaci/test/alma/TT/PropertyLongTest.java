 ...
 * @version $id$
 */
public class PropertyLongTest extends TestCase {
...
System.out.println(timeFormatter.format(new Date(UTCUtility.utcOmgToJava(completion.timeStamp))) + " Value: " + value);
	 ...
	 */
	private ROlong ROproperty = null;
	private RWlong RWproperty = null;
	 ...
	 */
	private static final String COMPONENT_NAME = "LONGTESTING"; 
	...
	protected void setUp() throws Exception {
		...
		LongComponent lc = LongComponentHelper.narrow(obj);		
		ROproperty = lc.longReadback();
		RWproperty = lc.longCurrent();
	}
	...
	public void testCharacteristics() throws Throwable
	{
		...
		// RWdouble
		assertEquals(0, RWproperty.min_value(), 0);
		assertEquals(1000, RWproperty.max_value(), 0);
		

		assertEquals("longReadback", ROproperty.name());
		...
		assertEquals(0, ROproperty.default_value(), 0);
		
		assertEquals(1526, ROproperty.min_delta_trigger(), 0);
		assertEquals(0, ROproperty.graph_min(), 0);
		assertEquals(1000, ROproperty.graph_max(), 0);
		assertEquals(1, ROproperty.min_step(), 0);

		assertEquals(10, ROproperty.alarm_low_on(), 0);
		assertEquals(20, ROproperty.alarm_low_off(), 0);
		assertEquals(990, ROproperty.alarm_high_on(), 0);
		assertEquals(980, ROproperty.alarm_high_off(), 0);
	...
	public void testSetAsync() {
		...
		synchronized(cb)
		{
			RWproperty.set_async(500,cb._this(orb), descIn);
			try
			{
				// wait for 5s
				cb.wait(5000);
	...
	public void testGetHistory() {
		...
		longSeqHolder dsh = new longSeqHolder();
	...
	public void testCreateMonitor() {
		...
		Monitorlong monitor = ROproperty.create_monitor(cb._this(orb), descIn);
		try
		{
			// 10.5 sec
			Thread.sleep(10500);
...
ROproperty.alarm_high_on();
	...
	*/
	...
	public static TestSuite suite() {
		return new TestSuite(PropertyLongTest.class);
	}

	public static void main(String[] args) {
		junit.textui.TestRunner.run(PropertyLongTest.class);
		System.exit(0);
	}
