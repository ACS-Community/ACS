/*
 * AddSubCategoriesAction.java
 *
 * Created on October 28, 2004, 4:42 PM
 */
 ...
 * @author  woloszyn
 */
public class AddSubCategoriesAction extends NodeAction {
    protected static final Logger logger = LogFactory.getLogger(AddSubCategoriesAction.class.getName());    
    private static final String label = NbBundle.getMessage(AddSubCategoriesAction.class,"LBL_AddSubCategoriesAction_name");
    
    /** Creates a new instance of AddCategoryAction */
    public AddSubCategoriesAction() {
    }
        ...
        if ( registry.getActivated() instanceof ConsoleConfigurationWindow ) {
            ...
            for (int ix = 0; ix < activatedNodes.length; ix++) {
                try {
                    ...
                    confWindow.addSubCategories(gpNode);
            ...
            if ( registry.getActivated() instanceof CategorySelectorWindow ) {
                ...
                for (int ix = 0; ix < activatedNodes.length; ix++) {
                    try {
                        GPNode gpNode = activatedNodes[ix];
                        selectionWindow.addSubCategories(gpNode);
