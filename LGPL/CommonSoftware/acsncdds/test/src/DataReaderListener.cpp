...
#include "temperatureDataBlockEventTypeSupportC.h"
  ...
  throw (CORBA::SystemException)
{
  num_reads_ ++;
  try {
    ::TESTFRIDGE::temperatureDataBlockEventDataReader_var message_dr =
        ::TESTFRIDGE::temperatureDataBlockEventDataReader::_narrow(reader);
    ...
    if (status == DDS::RETCODE_OK) {
      ...
      if (si.valid_data == 1)
      {
        cout << "Message: key      = " << message.key << endl
          << "         absolutDiff = " << message.absolutDiff   << endl
          << "         status      = " << message.status   << endl;
      }
