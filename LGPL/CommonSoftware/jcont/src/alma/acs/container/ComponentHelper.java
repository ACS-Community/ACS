...
public abstract class ComponentHelper
{
	 ...
	 *           and the functional interface.
 	 * @throws ContainerException  if the component implementation construction failed or 
 	 *                              if the component does not implement its declared functional interface. 
 	 */
	protected final synchronized ComponentLifecycle getComponentImpl() throws ContainerException
	{
		if (m_componentImpl== null)
		{
			try {
				...
				throw new ContainerException("failed to create component implementation.", thr);
			}
			if (m_componentImpl == null)
			{
				throw new ContainerException("failed to create component implementation: null");
			}
			else if (!getInternalInterface().isInstance(m_componentImpl))
			{
				throw new ContainerException("component impl class '" + m_componentImpl.getClass().getName() + 
	...
	{	
		...
		try {
			...
			throw new ContainerException("failed to obtain the POATie class from component helper.", ex);
		}
		if (poaTieClass == null)
		{
		...
		{
			throw new ContainerException("poaTie does not implement operations interface");
		}
		return poaTieClass;
	}
	...
	final Class getOperationsInterface() throws ContainerException
	{
		...
		{
			throw new ContainerException("failed to retrieve 'operations' interface from component helper", ex);
		}
		return opIF;
	}
			...
			throws ContainerException
	{
		...
		try {
			...
			throw new ContainerException("failed to obtain the custom interface translator.", ex);
		}
