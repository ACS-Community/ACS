...
/**
 * This class can be used as a convenient map that keeps {@link RepeatGuard} objects associated with {@code String IDs}.
 ...
 * or alternatively you can use one instance of {@code MultipleRepeatGuard} and take the component names as {@code ID}s.
 * <p>
...
public class MultipleRepeatGuard
{
	...
	/**
	 * Constructor for a {@code MultipleRepeatGuard} with a bounded RepeatGuard cache.
	 * <p>
	 ...
	 * @param maxCacheSize
	 *        Number of {@code ID}s (as used in ..) whose repetition we'll keep track of.
	 ...
	 *        as the matching {@code RepeatGuard}s will be lost and re-created.
	...
	/**
	 * Constructor for a {@code MultipleRepeatGuard} without a bounded RepeatGuard cache 
	...
	public boolean hasRepeatGuard(String ID) {
		return guards.containsKey(ID);
	}
	
	public RepeatGuard getRepeatGuard(String ID) {
		return getOrCreateRepeatGuard(ID);
	}
	
	/**
	 * Sets a possibly modified {@code RepeatGuard} for the given {@code ID}, 
	 * which allows for some {@code RepeatGuard}s having different settings than those passed in 
	...
	private static class BoundedHashMap<K,V> extends LinkedHashMap<K,V> {
		...
		/**
		 * Constructor.
		 * The initial capacity of this map is determined as {@code Math.min(16, maxEntries)}.
		 ...
		 *            when the number of stored entries has reached {@code maxEntries} and a new entry is stored; 
		 ...
		 * overriding the {@code maxEntries} value given in the constructor
