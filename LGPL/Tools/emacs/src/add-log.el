;; add-log.el --- change log for Emacs with RCS and CVS version numbers
;;
;; "@(#) $Id: add-log.el,v 1.26 2002/02/27 10:32:53 vltsccm Exp $"
;;
;; who	     when       what
;; --------- ----------	-----------------------------------------------
;; ssandroc  21.10.1994	Copied from Emacs-19.27 destribution
;;			Removed prompting in 'prompt-for-change-log-name'
;;			always use the default filename.
;;           21.10.1994 Now the RCS-revision is included in the file-name;
;;			copied some parts from the RCS-package to achieve it.
;;			Replaced `find-change-log' by the previous version
;;			of Emacs-19.23, new one does not work as expected.
;; ssandroc  29.10.1994 Added Header, removed version of `find-change-log'.
;; ssandroc  29.10.1994 re-hilit the upper part to have correct colors.
;; ssandroc  14.02.1995 added support for cvs.el (include CVS version)
;; ssandroc  14.02.1995 added support tcl-mode (include proc name)
;; ssandroc  16.07.1995 added support for all [incr]Tcl keywords.
;; ssandroc  25.04.1996 added support for incrTcl/Tk 2.
;; ssandroc  04.10.1996 adapted to Emacs 19.33.
...
;;;}}} SSa: Added
;;;{{{ SSa: Added from RCS-package `add-log.el, v 1.15'
...
;;;{{{ SSa: Copied from `add-log.el' 19.23
;;;###autoload
...
;;;{{{ SSa: Added
	 ;; Find out the file-version from the mode-line string
	 (setq revision
	       (and (boundp 'vc-mode)
		    vc-mode
		    (let ((pos (string-match "[0-9]" vc-mode)))
		      (and pos
			   (substring vc-mode pos)))))
;;;}}} SSa: Replaced
	 (setq formatted-revision
...
;;;}}} SSa: Added from RCS-package `add-log.el, v 1.15'
;;;{{{ SSa: Added
	 (setq entry (concat entry formatted-revision))
;;;}}} SSa: Added
	 )
...
;;;{{{ SSa: Added
; re-hilit the upper part to have correct colors
  (hilit-rehighlight-region 1 300 t) ; start/end in chars (?), t==quiet
;;;}}} SSa: Added
  )
...
;;;{{{ SSa: Added
		 ...
		 ;; search for Tcl keyword start
		 (if (re-search-backward "^[ \t]*\\(public\\|protected\\|private\\)*[ \t]*\\(proc\\|method\\|variable\\|common\\|constructor\\|destructor\\|class\\|body\\|configbody\\)[^{]*" nil t)
		     (progn
		       ;;(skip-chars-forward " \t")
		       (re-search-forward "[ \t]*\\(public\\|protected\\|private\\)*[ \t]*" nil t)
		       (buffer-substring (point)
					 (progn 
					   (skip-chars-forward "^ \t")
					   (skip-chars-forward " \t")
