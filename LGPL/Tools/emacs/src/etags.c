...
struct	nd_st {			/* sorting structure			*/
	...
	char	*callername;		/* caller of function named     */
	char	*file;			/* file name			*/
...
int	bflag = 0;		/* -b: browser style output */
     ...
     char	*av[];
{
  ...
  if (ac <= 1)
    ...
    usage:
#ifdef VMS
      fprintf (stderr, "Usage: %s [-abetwvx] [-f outfile] file ...\n", progname);
#else
      fprintf (stderr, "Usage: %s [-BFabetuwvx] [-f outfile] file ...\n", progname);
#endif
      exit(BAD);
    }
     ...
     char *file;
{
  ...
  /* if not a .c or .h or .y or .C file, try fortran */
  if (cp && (cp[1] != 'c' && cp[1] != 'h' && cp[1] != 'y' && cp[1] != 'C')
      && cp[2] == '\0')
    {
      if (PF_funcs(inf) != 0)
	{
      ...
      rewind(inf);	/* no fortran tags found, try C or C++ */
    }
  if (bflag)
	{
	yyparse();
...
/* Record a tag on the current line.
  ...
  f is flag for type of record "f", "c", or "i". */

pfnote_c (name, f, lno, cno, callername)
     char *name;
     char f;
     int lno;
     long cno;
     char *callername;
{
  ...
  if (f != 'f' && f != 'c' && f != 'i') f = 'f'; /* default to function */
  np->f	= f;
  np->callername = savestr(callername);
  np->file = curfile;
  np->lno = lno;
  np->cno = cno;
  np->left = np->right = 0;
  if (head == NULL)
    ...
    add_node(np, head);
#ifdef DEBUG
  printf ("Recorded a tag named %s of type %c on line %d char pos %d\n",
        name, f, lno, cno);
  if (strcmp (callername, ""))
        printf ("Auxilliary field was %s\n", callername);
#endif
}
     ...
     NODE *node,*cur_node;
{
 ...
 if (!bflag)
  ...
  if (dif < 0) 
   ...
   {	/* See if the tag and caller fields are same. If so do not add. */
    if (0 == strcmp(node->callername, cur_node->callername))
	{
	return;
	}
   }
/* dif > 0 or caller is different. go ahead and add it */
  ...
  if (eflag)
    {
      if (!bflag)
        ...
        {
        fprintf (outf, "%s%c%d,%d,%c,%s\n",
           node->name, 0177, node->lno, node->cno, node->f, node->callername);
        }
    }
  else if (!xflag)
    {
     ...
     reg NODE *node;
{
  ...
  if (!bflag)
      ...
      {
         total += strlen (node->name) + 6; /* delimiters + \n */
         total += strlen (node->callername);
      }
