...
;; who      when     what
;;-------- -------- ------------------
;;mnastvog  03/30/95 changed for vlt
;;
;;
;; saveconf.el - save context file
;;
...
(defvar auto-save-and-recover-context 1
...
on terminals with a different number of rows and columns."
  ...
  (condition-case error-data
      (let (context-buffer mark user-context-file)	
          (setq user-context-file (concat (substitute-in-file-name "$HOME/")
				     ".emacs_" (user-login-name)))
	(if (not (file-writable-p user-context-file))
	    (if (file-writable-p (original-working-directory))
		(error "context is write-protected, %s" user-context-file)
	...
	(cond ((buffer-modified-p context-buffer)
	       ...
	       (set-buffer-modified-p nil)
	       ;; save it
	       (write-region (point-min) (point-max) user-context-file
			     nil 'quiet))
	      (t (condition-case data
		     (delete-file user-context-file) (error nil))))
...
point is set in each window to what is was when the context was saved."
  ...
  ;;
  (condition-case error-data
      (let (sexpr context-buffer user-context-file)
        (setq user-context-file (concat (substitute-in-file-name "$HOME/")
					".emacs_" (user-login-name)))
	(if (not (file-readable-p user-context-file))
	    (error "can't access context, %s" user-context-file))
	;;
	...
	(set-buffer context-buffer)
	(erase-buffer)
	(insert-file-contents user-context-file nil)
	;; so kill-buffer won't need confirmation later
	(set-buffer-modified-p nil)
	;;
	...
	(if (zerop (buffer-size))
	    (error "context file is empty, %s" user-context-file))
	;;
