...
# ALMA project
#
# "@(#) $Id: acsMakeManl.tcl,v 1.2 2009/05/11 15:18:03 eallaert Exp $"
#
...
#************************************************************************
#   NAME
#   acsMakeManl - do pattern replacements in a set of files
# 
#   SYNOPSIS
#   acsMakeManl ?-nobackup? ?-nobinary? ?-quiet? ?--? <pattern> <replacement> \
#                 <file1> ?<file2>? ...
# 
#   DESCRIPTION
#   acsMakeManl will search in all <files> for the <pattern> (a regular
...
#   unexpected results. Use the '-nobinary' option to let acsMakeManl skip
#   these files.
#
#   EXAMPLES
#   Replace all occurences of HIM by ME in all files in the current directory:
#       acsMakeManl -nobackup HIM ME *
#
#   Replace all occurences of HIM by ME in all files in the current directory
#   and it sibblings:
#       find . | xargs acsMakeManl -nobackup HIM ME 
#   Note that in this latter example, acsMakeManl will not prompt the user
...
while {![lempty $argv]} {
	...
	-* {
	    puts stderr "wrong option \"$arg\"; run acsMakeManl as";
	    puts stderr "acsMakeManl ?-nobackup? ?-nobinary? ?-quiet? ?--? <pattern> <replacement> <file1> ?<file2>? ...";
	    exit 1
	}
...
if {[llength $files] < 3} {
    puts stderr "wrong number of arguments; run acsMakeManl as"
    puts stderr "acsMakeManl ?-nobackup? ?-nobinary? ?-quiet? ?--? <pattern> <replacement> <file1> ?<file2>? ..."
    exit 1
}
