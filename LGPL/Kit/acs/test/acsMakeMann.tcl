...
# ALMA project
#
# "@(#) $Id: acsMakeMann.tcl,v 1.2 2009/05/11 15:18:03 eallaert Exp $"
#
...
#************************************************************************
#   NAME
#   acsMakeMann - do pattern replacements in a set of files
# 
#   SYNOPSIS
#   acsMakeMann ?-nobackup? ?-nobinary? ?-quiet? ?--? <pattern> <replacement> \
#                 <file1> ?<file2>? ...
# 
#   DESCRIPTION
#   acsMakeMann will search in all <files> for the <pattern> (a regular
...
#   unexpected results. Use the '-nobinary' option to let acsMakeMann skip
#   these files.
#
#   EXAMPLES
#   Replace all occurences of HIM by ME in all files in the current directory:
#       acsMakeMann -nobackup HIM ME *
#
#   Replace all occurences of HIM by ME in all files in the current directory
#   and it sibblings:
#       find . | xargs acsMakeMann -nobackup HIM ME 
#   Note that in this latter example, acsMakeMann will not prompt the user
...
while {![lempty $argv]} {
	...
	-* {
	    puts stderr "wrong option \"$arg\"; run acsMakeMann as";
	    puts stderr "acsMakeMann ?-nobackup? ?-nobinary? ?-quiet? ?--? <pattern> <replacement> <file1> ?<file2>? ...";
	    exit 1
	}
...
if {[llength $files] < 3} {
    puts stderr "wrong number of arguments; run acsMakeMann as"
    puts stderr "acsMakeMann ?-nobackup? ?-nobinary? ?-quiet? ?--? <pattern> <replacement> <file1> ?<file2>? ..."
    exit 1
}
