...
void finalize()
{
    ...
    ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Destroying CharacteristicComponents."));
	...
	try
	    ...
	    ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Destroying \"%s\"", devices[n]));
	    ...
	    ACS_SHORT_LOG((LM_INFO, 
			   "baciTestAlarmServer: Exceptions raised while destroying Component"));
	    // do not bail out just because of one Component destruction failure 
	    // ACE_CHECK_RETURN (-1);
	    }
	}

    ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Closing CORBA."));
...
int startBaciTestServer(char *szCmdLn)
{
	...
	ACS_SHORT_LOG((LM_INFO, "Logging proxy successfully created !"));
	ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Starting"));

	if(argc>=2)
	    {
	    monitoring = atoi(argv[1]);
	    ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: monitoring flag: %d passed from cmd line", monitoring));
	    }

	// Installs termination signal handlers, to be able
	// to terminate cleanly
	ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Installing signal handlers"));
	...
	ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: CharacteristicComponent IDs ready"));
	if (devCount<1) {
	...
	if (!ACSError::init (BACI_CORBA::getORB()))
	    {
	    ACS_LOG(LM_RUNTIME_CONTEXT, "baciTestAlarmServer",
		    (LM_INFO, "Failed to initialize ACSError."));
	    return -1;
	    }

	// read parameters
	ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Init DB"));
	...
	if (CORBA::is_nil(ps_obj.in()))
	    {
	    ACS_SHORT_LOG((LM_ERROR,"baciTestAlarmServer: Failed to activate \"%s\"", devices[n]));
	    delete ps;
	    delete cs_p[n];
	    finalize();
	...
	ps->_remove_ref();
        ...
        type[count] = CORBA::string_dup("BaciTestAlarmClass");
        count++;
 
        ACS_SHORT_LOG((LM_INFO, "baciTestAlarmServer: %s exported", devices[n]));
        
	}
	}
	catch (...) {
	ACS_SHORT_LOG((LM_INFO, "baciTestAlarmServer: Exceptions raised while creating Component"));
	return -1;
	//ACE_CHECK_RETURN (-1);
	}
	ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Writing IORS to file..."));
 
	write_iors_to_file(count, name, type, ior);
 
 	
	ACS_SHORT_LOG((LM_INFO,"baciTestAlarmServer: Running ORB."));
